...
using Helios.Zero.Application.Tests.TestDatas;
...
namespace Helios.Zero.Application.Tests
{
    ...
    /// Provides methods to easily work with <see cref="HeliosZeroDbContext"/>.
    /// </summary>
    public abstract class AppTestBase : AbpIntegratedTestBase<HeliosZeroTestModule>
    {
        ...
        {
            void NormalizeDbContext(HeliosZeroDbContext context)
            {
        ...
        protected void UsingDbContext(Action<HeliosZeroDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<HeliosZeroDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<HeliosZeroDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<HeliosZeroDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<HeliosZeroDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<HeliosZeroDbContext>())
                {
                    action(context);
                    context.SaveChanges();
                }
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<HeliosZeroDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<HeliosZeroDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<HeliosZeroDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<HeliosZeroDbContext, Task<T>> func)
        {
            T result;
